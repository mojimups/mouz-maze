...
public class Level2 extends BasePanel implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	
	//******************************************************
	static String lvlCode = "l2";
	public static String lvlNum = "Ⅱ";
	public static String lvlName = "Onion";
	static int star2 = 11;
	static int star3 = 7;
	private int xOff = 0;
	private int yOff = 25;
	...
	public Level2(){
		...
		GameFrame.frame.setFocusable(true);
			...
			gamePN.add(start);
				...
				if (scaleBool) {
					start.setBounds(((getWidth()/2)+733)+xOff, 346+yOff, 32, 32);
				}
				else {
					start.setBounds(((getWidth()/2)+713)+xOff, 351+yOff, 32, 32);
				}
			gamePN.add(end);
				end.addMouseListener(this);
				if (scaleBool) {
					end.setBounds(((getWidth()/2)+815)+xOff, 180+yOff, 32, 32);
				}
				else {
					end.setBounds(((getWidth()/2)+795)+xOff, 185+yOff, 32, 32);
				}
			//******************************************************
	}
	...
	protected void paintComponent(Graphics g) {
		...
		if (ended) {
			...
			g.fillOval(((getWidth()/2)-320)+xOff, ((getHeight()/2)-240)+yOff, 320*2, 240*2);
			g.setColor(KColor.bg);
			g.fillOval(((getWidth()/2)-266)+xOff, ((getHeight()/2)-200)+yOff, 266*2, 200*2);
			if (running) {
				g.setColor(KColor.gray);
			}
			else {
			...
			g.fillOval(((getWidth()/2)-213)+xOff, ((getHeight()/2)-160)+yOff, 213*2, 160*2);
			g.setColor(KColor.bg);
			g.fillOval(((getWidth()/2)-160)+xOff, ((getHeight()/2)-120)+yOff, 160*2, 120*2);
			if (running) {
				g.setColor(KColor.gray);
			}
			else {
			...
			g.fillRect(((getWidth()/2)+25)+xOff, 278+yOff, 50, 100);
			g.fillRect(((getWidth()/2)-75)+xOff, 196+yOff, 50, 100);
			g.fillOval(((getWidth()/2)-106)+xOff, ((getHeight()/2)-80)+yOff, 106*2, 80*2);
			g.setColor(KColor.bg);
			g.fillOval(((getWidth()/2)-53)+xOff, ((getHeight()/2)-40)+yOff, 53*2, 40*2);
			g.fillRect(((getWidth()/2)-25)+xOff, 135+yOff, 50, 300);
			//******************************************************
		}
	}
	@Override
